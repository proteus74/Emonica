#pragma once
void Set_Button_CC_Controller()
{
	int seltemp = -1;
	int temp = Presets[CurrentPreset].Buttons[ButtonToConfigure].CC_Controller;
	...
	while (exit == false)
	{
		if (ValueUp.Update() || ValueDown.Update())
		{
			...
			display.setFont(&FreeSansBold18pt7b);
			Presets[CurrentPreset].Buttons[ButtonToConfigure].CC_Controller = temp;
			printCentered(String(temp), 48);
			DrawProgressbar(temp, MinVal, MaxVal);
			display.display();
		}
		...
		if (Next.Update())
		{
			...
			// Hier Erhöhen
			currentScreen = Page_Set_Button_Type_CC_Min_Value;
		}
